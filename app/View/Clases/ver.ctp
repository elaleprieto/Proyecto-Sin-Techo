<div class="row-fluid">
	<div class="span12">
		...
		foreach ($clase['Etapa'] as $etapa):
			echo $this->Html->link('<span class="badge badge-info">'.$i++.'</span> ', array('controller'=>'etapas', 'action'=>'ver', $etapa['id']), array('escape'=>FALSE));
		endforeach
		?>
	</div>
</div>
<div class="row-fluid">
	<div class="span12">
		<h2>
			Bienvenido <?php echo $this->Session->read('Auth.User.name') ?>
			a <?php echo $clase['Clase']['name'] ?>
		</h2>
	</div>
</div>
<div class="row-fluid">
	<div class="span12">
		<?php echo $clase['Clase']['description'] ?>
		<!-- <?php foreach ($clase['Etapa'] as $etapa): ?>
			<dl class="dl-horizontal">
				<dt>
					<?php echo $this->Html->link(h($etapa['name']), array('controller'=>'etapas', 'action' => 'ver', $etapa['id'])) ?>
				</dt>
				<dd>
					<?php echo h($etapa['description']) ?>
				</dd>
			</dl>
		<?php endforeach; ?> -->
	</div>
...
<div class="row-fluid">
	<div class="span12 text-right">
		<?php
			echo $this->Html->link('<i class="icon-chevron-right"></i>', array('controller'=>'etapas', 'action'=>'ver', $clase['Etapa'][0]['id']), array('escape'=>FALSE))
		?>
	</div>
</div>
